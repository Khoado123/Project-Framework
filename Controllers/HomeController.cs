...
namespace Project_KFC_WEB.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult Cart(int page = 1, bool success = false, bool checkUser = true, bool checkProfile= true)
        {
            ...
            if (!checkProfile)
            {
                ViewBag.errorProfile = "Bạn cần nhập đầy đủ thông tin trong profile để thanh toán.";
            }
        ...
        //Thêm products từ giỏ hàng tạm thời vào database
        public ActionResult InsertCart()
        {
            ...
            //string userAddress = Session["address"] as string;

            //if (userName == null)
            //{
            //    // Kiểm tra userAddress nếu userName không tồn tại
            //    return RedirectToAction("Cart", "Home", new { checkUser = false});
            //}
            //else if (userAddress == null)
            //{
            //    // Kiểm tra userAddress nếu userName tồn tại
            //    return RedirectToAction("Cart", "Home", new { checkAddress = false });
            //}
            var user = db.accounts.FirstOrDefault(u => u.userName == userName);

            if (user == null || string.IsNullOrEmpty(user.address) || string.IsNullOrEmpty(user.name) || string.IsNullOrEmpty(user.phone))
            {
                return RedirectToAction("Cart", "Home", new { checkProfile = false });
            }
